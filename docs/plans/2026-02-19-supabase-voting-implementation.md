# Supabase Voting Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace hardcoded memes with real Supabase data and allow authenticated users to insert votes into `caption_votes`.

**Architecture:** Protect `/` via middleware, then rewrite `page.tsx` as a client component that fetches public images+captions on mount, gets the auth user's id, and inserts a `caption_votes` row on each like/dislike click before advancing the UI.

**Tech Stack:** Next.js 16, React 19, @supabase/ssr, @supabase/supabase-js, Tailwind CSS v4, TypeScript

---

### Task 1: Protect the `/` route
...
git commit -m "feat: protect / route behind auth"
```

---

### Task 2: Rewrite page.tsx to fetch from Supabase and vote

**Files:**
- Modify: `src/app/page.tsx`
- Delete: `src/app/data/memes.ts`
...
export default function Home() {
  ...
  const handleVote = useCallback(
    async (vote: "like" | "dislike") => {
      ...
      // Insert vote into Supabase (fire and forget — don't block UI)
      if (userId) {
        const supabase = createClient();
      ...
      // Advance to next caption or next meme
      if (!isLastCaption) {
...
**Step 2: Delete the now-unused memes.ts**

```bash
rm src/app/data/memes.ts
```

**Step 3: Update next.config.ts to allow Supabase image domains**

Open `next.config.ts`. The `images.almostcrackd.ai` domain is not yet allowed. Replace the `remotePatterns` array:
...
git commit -m "feat: fetch images+captions from Supabase and insert caption_votes on vote"
```
...
If build/lint produced fixes, commit them:
...
**Step 3: Sign in and verify meme viewer**

Sign in with Google. Expected:
- Redirected back to `/`
- Memes load from Supabase (real images appear)
- Caption text shows below each meme
- Like/dislike buttons advance through captions
- After last caption, slides to next meme

**Step 4: Verify vote is recorded**

After clicking like or dislike, go to `http://localhost:3000/list`, select the `caption_votes` table.
Expected: A new row appears with your `profile_id`, the `caption_id`, and `vote_value` of `1` or `-1`.
